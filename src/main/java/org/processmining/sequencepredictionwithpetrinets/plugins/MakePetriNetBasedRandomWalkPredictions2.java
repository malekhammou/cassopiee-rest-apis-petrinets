...
@Plugin(
		name = "Test Petri Net Classifier using OnlineConformance Prefix-Alignments", 
		parameterLabels = {"Log", "Model"}, 
	    returnLabels = {"XLog"}, 
	    returnTypes = { XLog.class }
		)
public class MakePetriNetBasedRandomWalkPredictions2 {
	private final int MAX_LOOKAHEAD = 1;
	private final int MONTE_CARLO_ITERATIONS = 1000; 
	
	@UITopiaVariant(affiliation = UITopiaVariant.EHV, author = "N. Tax", email = "n.tax@tue.nl")
	@PluginVariant(variantLabel = "Test Petri Net Classifier using OnlineConformance Prefix-Alignments", requiredParameterLabels = {0, 1})
		...
		for(XTrace trace : testLogClone){
			...
			for(int prefixLength=0; prefixLength<trace.size(); prefixLength++){
				...
				if(prefixLength>0){
					...
					IncrementalReplayResult<String, String, Transition, Marking, PartialAlignment<String, Transition, Marking>> alignment = AlignmentUtils.calculateAlignment(context, prefixForalignmentLog, apn, replayer, false);
